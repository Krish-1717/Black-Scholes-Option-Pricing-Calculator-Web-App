...
with tab_fc:
    ...
    if result is None:
        ...
        fig = go.Figure()
        # Historical fitted vol (last 60 days)
        ...
        v1.metric("Daily Vol (σ)", f"{var_info['daily_vol']:.4f}")
        v2.metric(f"{confidence:.1%} VaR (% of portfolio)", f"{var_info['var_pct']:.2%}")
        v3.metric(f"{horizon}-day VaR ($)", f"${var_info['var_dollar']:,.0f}")
...
with tab_diag:
    ...
    if result is None:
        ...
        fig.add_trace(go.Scatter(y=std_res, mode="lines",
                                 line=dict(color="#a0a0a0", width=0.8), name="ẑ"), row=1, col=1)
